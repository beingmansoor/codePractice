...
public class CheckTree {
	...
	public static boolean detectCycle(Graph g) {
		...
		// Holds a flag if the node is currently in Stack or not (by default-
		// false)
	...
	public static boolean checkOneParent(Graph g) {
		...
		for (int i = 0; i < num_of_vertices; i++) {
			...
			if (g.adjacencyList[i] != null) {
				...
				while (tmp != null) {
					if (hasOneParent[tmp.data]) // if a node has more than one
												// parent
	...
	private static boolean checkConnected(Graph g, int source) {
		...
		// Create Stack(Implemented in previous section) for Depth First
		// Traversal and Push source in it
		...
		while (!stack.isEmpty()) {
			...
			// and push unvisited vertices in stack and also increment
			// vertices_reached
			DoublyLinkedList<Integer>.Node temp = null;
		...
		// +1 for source, and if number of vertices reachable from source are
		// equal
