...
class TreeBoundary {
	...
	public static List<TreeNode> findBoundary(TreeNode root) {
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < levelSize; i++) {
				...
				if (currentNode.left == null && currentNode.right == null) // skip
																			// leaf
																			// nodes
					continue;
				else if (i == 0) // if it is the first node of this level, add
									// it to the leftView
					leftView.add(currentNode);
				else if (i == levelSize - 1) { // if it is the last node of this
												// level, add it to the
												// rightView
					// because of ant-clockwise direction, we need to populate
					// rightView in the reverse direction
	...
	public static List<TreeNode> findBoundary1(TreeNode root) {
