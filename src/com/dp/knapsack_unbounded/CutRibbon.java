...
public class CutRibbon {
	...
	public int countRibbonPiecesRec(int[] ribbonLengths, int total) {
	...
	public int countRibbonPieces(int[] ribbonLengths, int total) {
		...
		// populate the total=0 columns, as we don't need any ribbon to make
		// zero total
		for (int i = 0; i < n; i++)
			dp[i][0] = 0;
		...
		for (int i = 0; i < n; i++) {
			for (int t = 1; t <= total; t++) {
				if (i > 0) // exclude the ribbon
					dp[i][t] = dp[i - 1][t];
				// include the ribbon and check if the remaining length can be
				// cut into available lengths
				if (t >= ribbonLengths[i] && dp[i][t - ribbonLengths[i]] != Integer.MIN_VALUE)
					dp[i][t] = Math.max(dp[i][t], dp[i][t - ribbonLengths[i]] + 1);
			}
		}

		// total combinations will be at the bottom-right corner, return '-1' if
	...
	private int countRibbonPiecesRecursive(int[] ribbonLengths, int total, int currentIndex) {
		...
		// if the ribbon length at the currentIndex exceeds the total, we
