...
/*
 * Given a staircase with ‘n’ steps and an array of ‘n’ numbers representing the fee that you 
 * have to pay if you take the step. Implement a method to calculate the minimum fee required 
...
public class StaircaseWithFee {
	...
	public int findMinFee(int[] fee) {
		...
		// for 2 or 3 steps staircase, since we start from the first step so we
		// have to pay its fee
		// and from the first step we can reach the top by taking two or three
		// steps, so we don't
	...
	public int findMinFeeRecursive(int[] fee, int currentIndex) {
