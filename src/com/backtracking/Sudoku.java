...
public class Sudoku {
	...
	public void solve() {
		backtrack(board, 0, -1); // it is important to understand why we are
									// passing 0 as row
									// and -1 as column. For this, please see
									// how we are
									// calculating currentRow and currentCol in
									// backtrack( ... ) method
	}
	...
	public void backtrack(char[][] board, int row, int col) {
		...
		// we go to next row only when col has reached 8, otherwise row remains
		// same
		int currentCol = col == 8 ? 0 : col + 1; // column of current cell
		// new col resets to 0 if previous col has reached 8, otherwise to get
		// new col we just shift one cell towards right
		// which is same as incrementing the previous col by 1

		if (currentRow == 9) { // nextRow == 9 means we have already processed
								// cell [8, 8] which marks the end of Sudoku
								// game
			finished = true;
			return;
		}
		if (board[currentRow][currentCol] == '0') { // if current cell contains
													// '0', solve for the
													// current cell
			...
			for (int k = 1; k <= 9; k++) {
				if (suitableCandidatesForCurrentCell[k - 1]) {
					...
					backtrack(board, currentRow, currentCol); // recursively
																// call
																// backtrack(...)

					if (finished) { // early termination
						// the moment we get one valid solution for the Sudoku
						// we
						// do not bother to compute any more even the Sudoku may
						// have more than one solutions.
						//
						// Also notice that, specifically fo rthis solution,
						// this step
						// also avoids the undoMove() step when we are returning
						// back to the calling function (during recursion)
						// an avoiding undoMove is necessary to keep the
						// solution persisted in the board[][] array.
						return;
					}
	...
	private boolean[] constructCandidates(char[][] board, int row, int col) {
		...
		// once we have computed the row and col of the top left point of the
		// sub-box
