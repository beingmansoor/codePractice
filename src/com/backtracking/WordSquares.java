...
public class WordSquares {
	...
	public static void main(String[] args) {
		...
		System.out.println(w.wordSquares(new String[]{"ball","area","lead","lady"}));
	}
	public List<List<String>> wordSquares(String[] words) {
		...
		for (String word : words) {
			// We initialize a partial solution with the current word as
			// top-most word
	...
	private void backtrack(int row, List<String> partialSolution, List<List<String>> completeSolution, int N,
			String[] words, Trie trie) {
		if (row == N) {
		...
		// if we are currently at row R in the word square (looking foir
		// suitable words for row R),
		...
		Set<Integer> suitableCandidates = trie.getWordsWithPrefix(prefix.toString()); // construct
																						// suitable
																						// candidates
		...
		for (Integer wordIndex : suitableCandidates) {
			...
			backtrack(row + 1, partialSolution, completeSolution, N, words, trie); // recursively
																					// call
																					// backtrack(
																					// ...
																					// )
...
class TrieNode {
	 ...
	 * there is no need for looking up if a word exists in the trie. We are only
	 * interested in looking up all the words starting with a specific prefix.
	 */
	char ch;
	HashMap<Character, TrieNode> children = new HashMap<Character, TrieNode>();
	Set<Integer> wordList = new HashSet<Integer>();
...
class Trie {
	...
	public Set<Integer> getWordsWithPrefix(String prefix) {
