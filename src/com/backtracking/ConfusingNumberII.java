...
public class ConfusingNumberII {
	 ...
	 * Example 1:
	 * 
	 * Input: 20 Output: 6 Explanation: The confusing numbers are
	 * [6,9,10,16,18,19]. 6 converts to 9. 9 converts to 6. 10 converts to 01
	 * which is just 1. 16 converts to 91. 18 converts to 81. 19 converts to 61.
	 * Example 2:
	 * 
	...
	private void search(long num) {
		if (num > n)
			return;
		if (num != 0) {
			long rotatedNo = rotate(num);
			if (rotatedNo != num)
				System.out.println(num + "\t" + rotatedNo);
				++count;
		}
		if (num != 0)
			search(num * 10);
	...
	public static void main(String[] args) {
		int confusingNumberII = new ConfusingNumberII().confusingNumberII(200);
		System.out.println(confusingNumberII);
	}
}
