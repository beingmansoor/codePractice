...
public class RotationCountWithDuplicates {
	...
	public static int countRotations(int[] arr) {
		...
		while (start < end) {
			...
			if (mid < end && arr[mid] > arr[mid + 1]) // if element at mid is
														// greater than the next
														// element
				return mid + 1;
			if (mid > start && arr[mid - 1] > arr[mid]) // if element at mid is
														// smaller than the
														// previous element
				return mid;
			...
			// choose a side
			// the best we can do is to skip one number from both ends if they
			// are not the smallest number
