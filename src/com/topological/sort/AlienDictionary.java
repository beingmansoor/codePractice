 ...
 * There is a dictionary containing words from an alien language for which we don’t know the ordering 
 of the characters. Write a method to find the correct order of characters in the alien language.
...
From the above two points, we can conclude that the correct character order is: "ywxz"
 */
public class AlienDictionary {
  public static String findOrder(String[] words) {
	...
	/*
	 * Time complexity # In step ‘d’, each task can become a source only once
	 * and each edge (a rule) will be accessed and removed once. Therefore, the
	 * time complexity of the above algorithm will be O(V+E), where ‘V’ is
	 * the total number of different characters and ‘E’ is the total number of
	 * the rules in the alien language. Since, at most, each pair of words can
	 * give us one rule, therefore, we can conclude that the upper bound for the
	 * rules is O(N) where ‘N’ is the number of words in the input. So, we
	 * can say that the time complexity of our algorithm is O(V+N).
	 * 
