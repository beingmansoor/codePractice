...
public class NextInterval {
	public static int[] findNextInterval(Interval[] intervals) {
		...
		for (int i = 0; i < n; i++) {
			int topEnd = maxEndHeap.poll(); // let's find the next interval of
											// the interval which has the
											// highest 'end'
			...
			if (intervals[maxStartHeap.peek()].start >= intervals[topEnd].end) {
				...
				maxStartHeap.add(topStart); // put the interval back as it could
											// be the next interval of other
											// intervals
			}
		}
		return result;
