...
public class SlidingWindowMedian {
	 ...
	 * Example 2:
	 * 
	 * Input: nums=[1, 2, -1, 3, 5], k = 3 Output: [1.0, 2.0, 3.0] Explanation:
	 * Lets consider all windows of size ‘3’:
	 * 
	 * [1, 2, -1, 3, 5] -> median is 1.0 [1, 2, -1, 3, 5] -> median is 2.0 [1,
	 * 2, -1, 3, 5] -> median is 3.0
	 */
	...
	public double[] findSlidingWindowMedian(int[] nums, int k) {
		...
		for (int i = 0; i < nums.length; i++) {
			...
			if (i - k + 1 >= 0) { // if we have at least 'k' elements in the
									// sliding window
				// add the median to the the result array
				if (maxHeap.size() == minHeap.size()) {
					// we have even number of elements, take the average of
					// middle two elements
					result[i - k + 1] = maxHeap.peek() / 2.0 + minHeap.peek() / 2.0;
				} else { // because max-heap will have one more element than the
							// min-heap
