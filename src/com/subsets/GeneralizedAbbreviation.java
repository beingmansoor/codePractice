...
public class GeneralizedAbbreviation {
	...
	public static List<String> generateGeneralizedAbbreviation(String word) {
		...
		while (!queue.isEmpty()) {
			...
			if (abWord.start == wordLen) {
				...
				// continue abbreviating by incrementing the current
				// abbreviation count
				queue.add(new AbbreviatedWord(new StringBuilder(abWord.str), abWord.start + 1, abWord.count + 1));

				// restart abbreviating, append the count and the current
	...
	private static void generateAbbreviationRecursive(String word, StringBuilder abWord, int start, int count,
		...
		if (start == word.length()) {
			...
			// continue abbreviating by incrementing the current abbreviation
			// count
			generateAbbreviationRecursive(word, new StringBuilder(abWord), start + 1, count + 1, result);

			// restart abbreviating, append the count and the current character
			// to the string
			if (count != 0)
				abWord.append(count);
