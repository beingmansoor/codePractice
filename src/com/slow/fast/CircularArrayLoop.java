...
This problem involves finding a cycle in the array and, as we know, the Fast & Slow pointer
 method is an efficient way to do that. We can start from each index of the array to find the 
 cycle. If a number does not have a cycle we will move forward to the next element. 
...
This means that when we move a pointer forward, if the pointer points to the same element
 after the move, we have a one-element cycle. Therefore, we can finish our cycle search for
  the current element.
...
The other requirement mentioned in the problem is that the cycle should not contain both
 ...
 forward and if the number is negative, the direction will be backward. 
...
public class CircularArrayLoop {
	...
	public static boolean loopExists(int[] arr) {
		for (int i = 0; i < arr.length; i++) {
			...
			// this number
			do {
				slow = findNextIndex(arr, isForward, slow); // move one step for
															// slow pointer
				fast = findNextIndex(arr, isForward, fast); // move one step for
															// fast pointer
				if (fast != -1)
					fast = findNextIndex(arr, isForward, fast); // move another
																// step for fast
																// pointer
